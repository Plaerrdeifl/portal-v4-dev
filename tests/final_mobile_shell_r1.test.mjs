...
test("standalone PWA uses the full dynamic viewport", async () => {
  ...
  assert.match(
    css,
    /\.mobile-bottom-nav\{[\s\S]*bottom:0!important;/
  );
});
...
test("cache busting identifies final mobile shell R1", async () => {
  ...
  assert.match(index, /20260722-final-mobile-shell-r1/);
  assert.match(config, /20260722-final-mobile-shell-r1/);
  assert.match(worker, /pd-portal-v4-final-mobile-shell-r1-20260722/);
});
