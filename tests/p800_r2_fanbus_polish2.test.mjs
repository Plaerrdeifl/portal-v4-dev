...
test("Polish 2 keeps normal page scrolling while compacting the mobile trip stop editor", () => {
  ...
  assert.match(ux, /\.v4-m310-editor-stops\{gap:9px;padding:12px\}/);
  assert.match(ux, /\.v4-m310-trip-stop-editor-row\{[^}]*grid-template-columns:minmax\(0,1fr\) 104px auto;[^}]*gap:6px;[^}]*padding:8px/);
