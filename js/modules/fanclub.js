...
async function deleteAccount(id){if(!await confirmAction({title:"Konto endgültig löschen",message:"Nur ein deaktiviertes, unbenutztes Konto mit Saldo 0,00 € kann gelöscht werden.",confirmText:"Konto löschen"}))return;await runWrite("Konto wird gelöscht …",()=>call("apiDeleteAccount",id));phase3State.remove(KEY+"accounts");await renderAccounts(true);}
...
function openTaskForm(task = {}, meta = {}) {
  ...
  openDialog({
    title: task.row ? "Aufgabe bearbeiten" : "Aufgabe anlegen",
