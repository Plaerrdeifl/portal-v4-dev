...
function tabs(){ const p=portal(); return [
  {id:"overview",label:"Meine Teams",icon:"👥",show:true},
  {id:"tasks",label:"Aufgaben",icon:"✅",show:Boolean(p.teamMember)||Boolean(p.teamAccess)},
  {id:"manage",label:"Verwaltung",icon:"🛠️",show:Boolean(p.teamLeader)||Boolean(p.teamAdmin)},
  {id:"admin",label:"Team-Admin",icon:"⚙️",show:Boolean(p.teamAdmin)}
].filter(x=>x.show); }
...
function openTaskForm(task = {}, meta = {}) {
  ...
  openDialog({
    title: task.row ? "Aufgabe bearbeiten" : "Teamaufgabe anlegen",
    kicker: task.team || "Team",
    body: `<form>
      <input type="hidden" name="row" value="${escapeAttr(task.row || "")}">
...
async function openTeamDetails(teamId, meta, adminMode) {
      ...
      ${members.length ? `<div class="card table-card"><div class="data-table-wrap"><table class="data-table">
        ...
        <tbody>${members.map(member => `<tr>
          ...
          <td><div class="button-row">
            ...
            <button class="button small ${member.aktiv === "JA" ? "danger" : "secondary"}" data-toggle-team-member="${escapeAttr(member.id)}" data-active="${member.aktiv === "JA" ? "false" : "true"}">${member.aktiv === "JA" ? "Deaktivieren" : "Aktivieren"}</button>
          </div></td>
        </tr>`).join("")}</tbody>
  ...
  document.querySelectorAll("[data-toggle-team-member]").forEach(button => button.addEventListener("click", async () => {
    ...
    await runWrite(active ? "Teammitglied wird aktiviert …" : "Teammitglied wird deaktiviert …", () => call("apiSetTeamMemberActive", button.dataset.toggleTeamMember, active));
