...
import { queueM328FanbusAction } from "../m328-bus-orga-shell.js?v=20260829-m328-completion1";
...
function lifecycleBadge(value) {
  const type = value === "PUBLISHED" ? "success" : value === "DRAFT" ? "warning" : value === "CANCELLED" ? "danger" : "neutral";
...
function openLegacyTripAction(action, tripId) {
  queueM328FanbusAction(action, tripId);
  location.hash = "#/fanbuses?orga=1&from=bus-orga";
}

function actionButton(action, label, className = "secondary") {
  return `<button class="button small ${className}" type="button" data-trip-detail-action="${escapeAttr(action)}">${escapeHtml(label)}</button>`;
}
...
function managementActions(trip) {
  ...
  if (["DRAFT", "PUBLISHED"].includes(trip.status)) actions.push(actionButton("close", "Fahrt schließen", "ghost"));
  if (trip.status === "CLOSED") actions.push(actionButton("reopen", "Wieder als Entwurf öffnen"));
  if (trip.canCancel === true) actions.push(actionButton("cancel", "Fahrt absagen", "danger"));
  return actions.join("");
}
...
function renderPage(root, trip) {
  ...
  if (canRegistrations) {
    ...
    if (trip.status === "PUBLISHED") workingActions.push(actionButton("registration", "Anmeldung"));
    workingActions.push(actionButton("participants", "Teilnehmer"));
  }
  if (canManage) {
    workingActions.push(actionButton("occupancy", "Busse & Zuordnung"));
    if (["DRAFT", "PUBLISHED"].includes(trip.status)) workingActions.push(actionButton("edit", "Bearbeiten"));
  }
  if (canOperations) workingActions.push(actionButton("operations", "Fahrtbetrieb"));
  const lifecycleActions = managementActions(trip);
  root.innerHTML = `<div class="m328-trip-detail">
    <header class="m328-trip-detail-head"><button class="button small ghost" type="button" data-trip-detail-back>← Bus-Orga</button><div><span>${escapeHtml(formatDate(trip.eventDate))} · ${escapeHtml(formatTime(trip.eventTime))}</span><h2>${escapeHtml(venue)}</h2></div>${lifecycleBadge(trip.status)}</header>
    ...
    <section class="m328-trip-detail-panel"><h3>Fahrtdaten</h3><dl class="m328-trip-detail-facts">
      ...
      <div><dt>Anmeldeschluss</dt><dd>${escapeHtml(formatDateTime(trip.registrationClosesAt))}</dd></div>
      <div><dt>Belegung</dt><dd>${Number(trip.activeRegistrationCount || 0)} Teilnehmer · ${Number(trip.waitlistedRegistrationCount || 0)} Warteliste · ${Number(trip.remainingCapacity || 0)} frei</dd></div>
    </dl></section>
    <section class="m328-trip-detail-panel"><h3>Fahrt verwalten</h3><div class="m328-trip-detail-actions">${workingActions.length ? workingActions.join("") : '<p class="subtle">Für diese Fahrt sind keine Arbeitsbereiche freigeschaltet.</p>'}</div></section>
  ...
  root.querySelector("[data-trip-detail-back]")?.addEventListener("click", () => { location.hash = "#/bus-orga"; });
...
function ensureStyle() {
  ...
  style.textContent = `
    ...
    .m328-trip-detail-panel{display:grid;gap:9px;padding:11px;border:1px solid var(--line);border-radius:14px;background:var(--surface)}.m328-trip-detail-panel h3{margin:0;font-size:1rem}
    .m328-trip-detail-facts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:0}.m328-trip-detail-facts div{display:grid;gap:2px;padding:8px;border-radius:10px;background:var(--surface-2)}.m328-trip-detail-facts dt{color:var(--muted);font-size:.68rem;font-weight:750;text-transform:uppercase}.m328-trip-detail-facts dd{margin:0;font-size:.79rem;font-weight:750;line-height:1.3}
    .m328-trip-detail-actions{display:flex;flex-wrap:wrap;gap:7px}.m328-trip-detail-actions .button{min-height:42px}
    @media(max-width:620px){.m328-trip-detail-head{grid-template-columns:auto minmax(0,1fr)}.m328-trip-detail-head>.badge{grid-column:2;justify-self:start}.m328-trip-detail-facts{grid-template-columns:1fr}.m328-trip-detail-actions{display:grid;grid-template-columns:1fr 1fr}.m328-trip-detail-actions .button{width:100%}}
  `;
...
async function executeTripAction(action, trip, confirmation) {
  ...
  if (!confirmed) return;
  try {
    await runWrite(() => call(action, { id: trip.id, expectedRevision: Number(trip.revision) }), confirmation.success);
    ...
    showToast(error?.code === "40001" ? "Die Daten wurden zwischenzeitlich geändert. Bitte Ansicht aktualisieren." : error?.message || "Die Fanbus-Aktion konnte nicht ausgeführt werden.", "error", 5200);
  }
}
...
async function handleAction(action, trip) {
  if (action === "bookings") return navigate("bookings", trip.id);
  if (action === "registration") return navigate("registration", trip.id);
  if (action === "edit") return navigate("trip-edit", trip.id);
  if (action === "participants" || action === "occupancy") return openLegacyTripAction(action, trip.id);
  if (action === "operations") {
    location.hash = `#/fanbuses?view=operations&trip=${encodeURIComponent(trip.id)}&fromTrip=${encodeURIComponent(trip.id)}&from=bus-orga`;
    return;
  }
  if (action === "cancel") return openCancellation(trip);
  const confirmations = {
    publish: { call: "fanbus_trip_publish", message: `Fanbusfahrt „${trip.displayTitle || "Fanbusfahrt"}“ veröffentlichen?`, options: { title: "Fanbusfahrt veröffentlichen", submitLabel: "Veröffentlichen" }, success: "Fanbusfahrt wurde veröffentlicht." },
    close: { call: "fanbus_trip_close", message: `Fanbusfahrt „${trip.displayTitle || "Fanbusfahrt"}“ endgültig schließen?`, options: { danger: true, title: "Fanbusfahrt schließen", submitLabel: "Fahrt schließen" }, success: "Fanbusfahrt wurde geschlossen." },
    reopen: { call: "fanbus_trip_reopen", message: "Die Fanbusfahrt wird wieder als Entwurf geöffnet. Sie ist danach nicht öffentlich verfügbar und kann wieder bearbeitet werden.", options: { title: "Fanbusfahrt wieder öffnen", submitLabel: "Als Entwurf öffnen" }, success: "Fanbusfahrt wurde wieder als Entwurf geöffnet." },
    delete: { call: "fanbus_trip_delete", message: `Entwurf „${trip.displayTitle || "Fanbusfahrt"}“ endgültig löschen?`, options: { danger: true, title: "Fanbus-Entwurf löschen", submitLabel: "Entwurf löschen" }, success: "Fanbus-Entwurf wurde gelöscht." }
  };
...
export async function hydrateBusOrgaTripDetail(context = {}) {
  ...
  try {
    ...
    root.querySelector("[data-trip-detail-load-back]")?.addEventListener("click", () => { location.hash = "#/bus-orga"; });
  }
}
