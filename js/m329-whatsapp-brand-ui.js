...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .m329-contact-action.m329-whatsapp .m329-whatsapp-brand-mark{
...
function bindDirectWhatsAppNavigation(button) {
  if (!(button instanceof HTMLAnchorElement)) return;
  const href = safeWhatsAppHref(button.getAttribute("href"));
  if (!href) return;
  button.removeAttribute("target");
  button.setAttribute("rel", "noopener");
  if (button.dataset.m329WhatsAppNavigation === "true") return;
  button.dataset.m329WhatsAppNavigation = "true";
...
function applyBrandMark() {
  ...
  document.querySelectorAll(BRAND_SELECTOR).forEach(button => {
    ...
    bindDirectWhatsAppNavigation(button);
