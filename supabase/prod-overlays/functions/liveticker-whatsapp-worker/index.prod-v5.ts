...
async function claim() {
  ...
  for (let pass = 0; pass < 8; pass += 1) {
    ...
    const targets = await rest(targetViewUrl({
      select: "id,job_id,channel_code,channel_label,waha_channel_id,status,attempt_count,claimed_at,worker_received_at,sticker_status,text_status,sticker_waha_message_id,sticker_sent_at,text_waha_message_id,text_sent_at,created_at",
      attempt_count: "lt.5",
      status: "eq.PENDING",
    ...
    const jobs = await rest(viewUrl({
      select: "id,event_id,client_action_id,publication_version,message,sticker_id,delivery_mode,linked_action_id,created_at",
      id: `eq.${jobId}`,
      limit: "1",
    }));
...
async function targetById(jobId: string, targetId: string) {
  const rows = await rest(targetViewUrl({
    select: "id,job_id,status,attempt_count,sticker_status,text_status,sticker_waha_message_id,sticker_sent_at,text_waha_message_id,text_sent_at",
    id: `eq.${targetId}`,
    job_id: `eq.${jobId}`,
    limit: "1",
  }));
