...
-- P300 / M325-R1 F5 Round 2: authoritative capacity from active trip buses.

create or replace function app_private.fanbus_effective_capacity(p_trip_id uuid)
returns integer
language sql
stable
security definer
set search_path = ''
as $$
  select coalesce(sum(bus.capacity), 0)::integer
  from app_modules.fanbus_buses as bus
  where bus.trip_id = p_trip_id
    and bus.is_active;
$$;

revoke all on function app_private.fanbus_effective_capacity(uuid)
  ...
  return jsonb_build_object(
    ...
    'summary', (
      select jsonb_build_object(
        ...
        'activeBusCapacity', app_private.fanbus_effective_capacity(v_trip_id)
      )
      from app_modules.fanbus_registrations as registration
      where registration.trip_id = v_trip_id
    )
...
create or replace function app_private.api_fanbus_waitlist_promote(p_payload jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  v_capacity := app_private.fanbus_effective_capacity(v_trip_id);
  select * into v_existing
...
begin
  ...
  -- Serialize every effective-capacity change with booking and promotion decisions.
  perform 1 from app_modules.fanbus_trips where id = v_trip_id for update;
  if not found then
    raise exception 'Die Fanbusfahrt wurde nicht gefunden.' using errcode = 'P0002';
  end if;
...
create or replace function app_private.api_fanbus_buses_list(p_payload jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
