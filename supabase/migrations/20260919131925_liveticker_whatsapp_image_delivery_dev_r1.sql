-- DEV only: add durable PNG image delivery for Liveticker summary flyers.
-- Existing text/sticker jobs keep their current behavior.
...
alter table app_modules.liveticker_whatsapp_jobs
  ...
  add column image_status text not null default 'NOT_REQUESTED',
  add column image_waha_message_id text,
  add column image_sent_at timestamptz;

alter table app_modules.liveticker_whatsapp_jobs
  drop constraint liveticker_whatsapp_jobs_delivery_mode_check,
  drop constraint liveticker_whatsapp_jobs_component_status_check,
  drop constraint liveticker_whatsapp_jobs_components_check,
  drop constraint liveticker_whatsapp_jobs_delivery_success_check;

alter table app_modules.liveticker_whatsapp_jobs
  add constraint liveticker_whatsapp_jobs_delivery_mode_check
    check (delivery_mode in ('TEXT_ONLY','STICKER_THEN_TEXT','STICKER_ONLY','IMAGE_ONLY')),
...
begin
  ...
  if app_private.platform_release_environment() is distinct from 'DEV'
     ...
     or p_payload - array['eventId','deliveryMode','idempotencyKey','imageUrl','imageFilename']::text[] <> '{}'::jsonb
     or not (p_payload ?& array['eventId','deliveryMode','idempotencyKey','imageUrl','imageFilename']::text[])
     ...
     or v_image_filename !~ '^[A-Za-z0-9._-]{1,96}[.]png$' then
    raise exception 'LIVETICKER_WHATSAPP_IMAGE_ENQUEUE_INVALID' using errcode = '22023';
  end if;

  v_event_id := (p_payload ->> 'eventId')::uuid;
  v_idempotency_key := (p_payload ->> 'idempotencyKey')::uuid;
  v_request_key := 'image:' || v_idempotency_key::text;
  ...
  if not found
    ...
    raise exception 'LIVETICKER_WHATSAPP_IMAGE_ENQUEUE_CONFLICT' using errcode = '23505';
  end if;
