...
begin
  ...
  return jsonb_build_object('games',coalesce((
    select jsonb_agg(x.item order by x.event_date desc,x.event_time desc nulls last,x.event_id) from (
      select e.id event_id,e.event_date,e.event_time,
        jsonb_build_object(
...
grant execute on function public.pd_public_liveticker_graphics_games() to authenticated;
