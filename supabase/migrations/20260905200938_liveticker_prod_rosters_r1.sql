-- Plärrdeifl Portal V4 - frozen Bayernliga roster seed for PROD
DO $$
DECLARE
  v_teams jsonb := $teams${
...
BEGIN
  ...
  LOOP
    SELECT team.id INTO v_team_id FROM app_modules.liveticker_teams team
    ...
    ELSE
      UPDATE app_modules.liveticker_teams SET short_name=v_team->>'short',is_active=true,updated_at=now()
      WHERE id=v_team_id AND NOT is_home_club;
    END IF;
  END LOOP;
...
$roster$, E'\n')
  LOOP
    ...
    IF array_length(v_parts, 1) <> 4 THEN RAISE EXCEPTION 'LIVETICKER_ROSTER_SEED_INVALID_LINE: %', v_line; END IF;
    v_team := v_teams -> v_parts[1];
    v_team_name := v_team ->> 'name';
    v_number := nullif(btrim(v_parts[2]), '');
    SELECT team.id INTO v_team_id FROM app_modules.liveticker_teams team WHERE lower(btrim(team.name))=lower(btrim(v_team_name));
    IF v_team_id IS NULL THEN RAISE EXCEPTION 'LIVETICKER_ROSTER_SEED_TEAM_MISSING: %',v_team_name; END IF;
    IF NOT EXISTS (SELECT 1 FROM app_modules.liveticker_players player WHERE player.team_id=v_team_id AND lower(btrim(player.full_name))=lower(btrim(v_parts[3]))) THEN
      INSERT INTO app_modules.liveticker_players(team_id,full_name,jersey_number,position,is_active)
      VALUES(v_team_id,btrim(v_parts[3]),v_number,btrim(v_parts[4]),true);
    END IF;
  END LOOP;
END;
$$;
